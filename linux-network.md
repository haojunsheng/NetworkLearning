...
客观来讲，这本书没有讲 why，更多是 how，不过相当不错了。

给出了很多复杂概念的来龙去脉，比如 epoll，同步阻塞，socket ，time_wait，零拷贝，一条tcp 链接究竟需要多大资源？

首先通过一张大图为我们描绘了网络包从网卡到内核经历了什么？这个过程需要我们了解中断的概念。

接下来，是内核是如何和用户进程进行协作。这个里面是最为核心的，很多优化是靠这个的。讲解了阻塞的概念，和阻塞的本质开销，epoll 性能牛逼的原因。

接下来讲解了内核是如何发送网络包的。这个里面会涉及到数据的多次拷贝。以及提高性能的零拷贝技术。

接下来讲解了 TCP 连接。明确了 TCP 链接是由（服务端 ip，服务端端口，客户端 ip，客户端端口）4 要素来定义的，任何一个不同，都可以表示一个新链接。明确了 TCP 链接建立的耗时，TCP链接消耗的内存，一台机器可以支持的 TCP 链接数量。
...
进程需要陷入内核判断数据是否准备好，如果没有准备好，需要让出 CPU，等后面有数据后，睡眠的进程再被唤醒。
...
- 用户进程内核态。调用epoll wait等函数时会将进程陷入内核态来执行。这部分代码负责查看接收队列，以及负责把当前进程阻塞掉，让出CPU。
...
以往每个 socket都要陷入到内核一次，需要频繁上下文切换。 epoll 模型，可以一次性把多个 socket 交给 epoll，如果有准备好的数据，就可以直接读取。在用户数量很大的时候，就可以减少上下文切换了
...
2. 同步阻塞IO都需要哪些开销?
   ...
   从CPU 开销角度，一次同步阻塞 IO 导致两次进程上下文切换开销。一次切换需要3-5 微秒的耗时。
...
2. cannot assign requested address” 这个报错你知道是怎么回事吗?该如何解決?

   客户端端口号用尽了。扩大可用端口范围、减小最大TIME_ WAT状态连接数量等方法都是可行的。

3. 一个客户端端又可以同时用在两条连接上吗？

   可以的。只要客户端 ip 或者服务端不一致即可。

4. 建立一条TCP连接需要消耗多长时间?

   约等于一个 RTT，但是网络不佳的话，就不可控了。
